...
export function StatCard({
  ...
  return (
    <div style={{
      ...
      padding: 'var(--space-5)', display: 'flex', flexDirection: 'column', gap: 'var(--space-4)', ...style,
    }} {...rest}>
      <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 'var(--space-3)' }}>
        <div style={{ minWidth: 0 }}>
          <div style={{ fontSize: 'var(--fs-sm)', fontWeight: 'var(--fw-semibold)', color: 'var(--text-muted)', letterSpacing: 'var(--ls-snug)' }}>{label}</div>
          <div style={{ marginTop: 6, fontSize: 'var(--fs-h1)', fontWeight: 'var(--fw-extra)', color: 'var(--text-strong)', letterSpacing: 'var(--ls-tight)', lineHeight: 1.05, fontVariantNumeric: 'tabular-nums' }}>{value}</div>
        </div>
        {icon && (
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 'var(--space-2)', flexWrap: 'wrap' }}>
          {delta != null && (
            <span style={{
              ...
              background: up ? 'var(--green-50)' : 'var(--red-50)',
              color: up ? 'var(--green-600)' : 'var(--red-600)',
              fontSize: 'var(--fs-xs)', fontWeight: 'var(--fw-bold)', fontVariantNumeric: 'tabular-nums',
            }}>
          ...
          {deltaLabel && delta != null && <span style={{ fontSize: 'var(--fs-xs)', color: 'var(--text-faint)' }}>{deltaLabel}</span>}
          {footnote && <span style={{ fontSize: 'var(--fs-xs)', color: 'var(--text-muted)' }}>{footnote}</span>}
        </div>
      )}

      {progress != null && <ProgressBar value={progress} variant={progressVariant || tone} size="sm" />}
    </div>
  );
}
