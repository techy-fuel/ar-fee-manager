      ...
      {hasHeader && (
        <header
          style={{
            ...
            gap: 'var(--space-4)', padding: `${pad === '0' ? 'var(--space-5)' : pad} ${pad === '0' ? 'var(--space-5)' : pad} 0`,
          }}
        >
          <div style={{ minWidth: 0 }}>
            {title && (
              <h3 style={{ margin: 0, fontSize: 'var(--fs-h4)', fontWeight: 'var(--fw-bold)', color: 'var(--text-strong)', letterSpacing: 'var(--ls-snug)' }}>
                {title}
              </h3>
            )}
            {subtitle && (
              <p style={{ margin: '4px 0 0', fontSize: 'var(--fs-sm)', color: 'var(--text-muted)' }}>{subtitle}</p>
            )}
          </div>
